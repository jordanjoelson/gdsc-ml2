...
class RandomAgent:
    """
    Baseline random agent for LunarLander-v2 (or any env with
    8 continuous state values and 4 discrete actions).
    ...
    def select_action(self, state: np.ndarray) -> int:
        ...
        Args:
            state: np.ndarray of shape (8,) — the environment observation
    ...
    def learn(self, state, action, reward, next_state, done):
        """
        Placeholder — will contain the DQN update logic later.
...
if __name__ == "__main__":
    ...
    print("(Random baseline — expect ~−200 to −100)")
    env.close()